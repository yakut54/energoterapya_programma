<template>
  <div class="player" :class="{'dop-video': $route.name === 'video' || $route.name === 'procedure'}"
		:style="{backgroundImage: `linear-gradient(2deg, ${dark}, ${light} 49%, ${dark} 103%)`}">

		<teleport to=".teleport" v-if="isMounted && $route.name !== 'audio'">
      <div class="play-button" v-if="!isPlay" :style="{backgroundImage: `radial-gradient(${dark}, #fff)`}">
				<i class="fa fa-play-circle-o" @click="play" :style="{color}"></i>
			</div>
    </teleport>

    <div class="left-player">
      <div class="inner-left-player">
        <i class="fa fa-play-circle-o" @click="play" v-if="!isPlay" :style="{color}"></i>
        <i class="fa fa-pause-circle-o" @click="pause" v-else :style="{color}"></i>
      </div>
    </div>
    <div class="center-player">
      <div class="inner-center-player">
        <div class="center-player-top">
          ...
          >
            <div
              class="fill-scale-bar"
							:style="{ width: `${progress}%`, background: color}"
            >
              <div
                class="fill-scale-info-block"
								:style="{ left: infoBlockLeft, display: infoBlockView }"
              >
        ...
        <div class="center-player-bottom">
          <div class="left-bottom" :style="{color}">
          ...
          <div class="right-bottom">
            ...
            <div class='volume'>
              <i class="fa fa-volume-off" :style="{color}"></i>
...
export default {
  ...
  setup(props){
		...
		const idx = +localStorage.getItem('idx') || 0
		const [dark, light] = store.state.config.data[idx].playerColorGragient
		const color = store.state.config.data[idx].strelka
    const PLAYER = ref()
		const scale = ref()
		const volume = ref()
		const current = ref(0)
...
@media (max-width: 320px) {
	...
	.left-player, .right-player {max-width: 34px;}
	.inner-left-player *{font-size: 26px;}
	.audio-block{padding-left: 3px;padding: 3px;}
	.player{margin-top: 5px;margin-bottom: 5px;}
	.video-bottom .player{margin-top: 0px;margin-bottom: 0px;}
	.center-player-bottom {padding: 0 6px 0px 4px;}
}
</style>